...
//   });
// });
const dom = document.querySelector('.love-text')
const str = `❤️❤️❤️ love Sun
虽然我们认识点时间还不是很长，但这段时间相处，让我觉得你就是我的那个人。

缘分让我们以伴郎伴娘的方式见面，似乎就是在暗示我们未来的关系。

见到你的第一眼 觉得你美丽可爱，相处之后又觉得善解人意、知书达理。现在我的心里都是你，不和你在一起时候想你，和你一起时候开心。你遇到问题的时候，想帮助你解决。你遇到危险/风险的时候，会很担心，很想挡在你的前面。每次和你视频或语音的内心总是美美的，每天都想看到你的样子或听到你的声音。我想这种感觉应该就是爱吧。你说 对不

佳丽，我自认为我一个靠谱的人。对待你我会全心全意，太多的空话我不想说。以后你就看我的表现。

佳丽，做我现在的女朋友，以后老婆，永远的佳丽宝贝好吗`;
...
document.querySelector('.envelope-wrapper .heart').addEventListener('click',() => {
  document.querySelector('.envelope-wrapper').classList.add('flap')
  playMusic()
})

document.querySelector('.envelope-wrapper .close-icon').addEventListener('click', () => {
  document.querySelector('.envelope-wrapper').classList.remove('flap')
})

document.querySelector('#mail').addEventListener('transitionend', (e) => {
...
document.querySelector('.do-btn').addEventListener('click', () => {
  ...
  mySwiper.slidePrev();
  const dom = document.querySelector('.page2 .text');
  const str = '永远爱佳丽宝贝！'
  writing(dom, str)
})


function writing(dom, str, index = 0, cb) {
    if (index < str.length) {
        // dom.innerHTML = dom.innerHTML.replace(/span-animation/g, '') + `<span class="span-animation">${str[index]}</span>`
        dom.innerHTML = dom.innerText + `<span class="span-animation">${str[index]}</span>`

        setTimeout(writing.bind(this), 200, dom, str, ++index, cb)
    }else {
      cb && cb()
    }
}
